<?php

require_once("Unittest\UnitTest_test.php"); 
require_once("Unittest\Logger_test.php"); 
require_once("Unittest\Model_test.php"); 
require_once("Unittest\GenHTML_test.php"); 
require_once("Unittest\FileBase_test.php"); 
require_once("Unittest\Handler_test.php");
require_once("Unittest\Type_test.php");
require_once("Unittest\View_test.php");
require_once("Unittest\Model_test_1.php"); 
require_once("Unittest\Model_test_2.php"); 
require_once("Unittest\FileBase_test_1.php"); 
require_once("Unittest\ModBase_test.php");
require_once("Unittest\Model_test_3.php"); 
require_once("Unittest\Type_test_1.php"); 
require_once("Unittest\FileBase_test_2.php");
require_once("Unittest\Model_test_4.php"); 

require_once ("Unittest\Class_code_test.php");

require_once("Unittest\Model_test_5.php");  
require_once("Unittest\Model_test_6.php");  

require_once("Unittest\GenHTML_test.php"); 

//require_once("SQLBase.php");

require_once('TestShow_test.php');
// require_once('Try_test.php');




?>